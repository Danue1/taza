...
//!
//! ```text
//! taza build [언어…] [--data <디렉터리>] [--out <디렉터리>] [--skip-archive]
//! ```
//! 언어를 주지 않으면 `<데이터>/languages/*` 전부를 만든다. 산출물은 모두 `<산출>` 아래에
//! 놓인다: `build/<이름>-words.tsv`(중간 점수표, 사람이 들여다보는 용도),
//! `packs/<이름>.tazapack`, 같은 자리에 `.zst` 아카이브와 `catalog.json`.
//! 예외는 `<데이터>/notices/NOTICE.md` 하나다 — 저작자 표시는 저장소에 남아야 한다.
//!
//! 원천 파일은 `<산출>/cache/downloads`에 sha256과 함께 남으므로 다시 실행해도 네트워크를
//! 타지 않는다. 원천이 조용히 바뀌면 해시 검증에서 멈춘다.
//!
//! 단계의 순서는 `taza_packbuild::pipeline`이 소유한다. 이 파일이 하는 일은 인자를 읽고
//! 각 단계가 낸 수를 사람이 읽는 말로 옮기는 것뿐이다.

use std::path::PathBuf;
use std::process::ExitCode;

use taza_packbuild::pipeline::{self, BuildOptions, BuildReport};

const USAGE: &str = "사용법: taza build [언어…] [--data <디렉터리>] [--out <디렉터리>] \
    [--skip-archive] [--no-cache]";
...
fn run(names: &[String], options: &BuildOptions) -> Result<(), String> {
    let languages = pipeline::language_directories(options, names)?;
    let mut catalog = None;
    for language in &languages {
        let outcome = pipeline::build(language, options)?;
