...
//! 전체 파이프라인은 `taza build`(레시피 기반)이며, 이 도구는 그 마지막 단계만 손으로
//! 돌려 보고 싶을 때 쓴다.
//!
//! 입력: `단어<TAB>점수` TSV, 선택적 bigram TSV(`앞단어<TAB>뒷단어<TAB>가중치`) —
//! 둘 다 빈 줄과 `#` 주석 허용 → 출력: 언어팩 바이너리.
...
const USAGE: &str = "사용법: taza compile <언어태그> <단어.tsv> <출력.tazapack> \
    [--bigrams <tsv>] [--hangul-jamo]";

pub fn main(arguments: &[String]) -> ExitCode {
    let [language, input_path, output_path, options @ ..] = arguments else {
        eprintln!("{USAGE}");
    ...
    let pack = match compile(language, &tsv, bigram_tsv.as_deref(), hangul_jamo) {
        Ok(pack) => pack,
        Err(message) => {
...
mod tests {
    ...
    fn compiles_tsv_into_pack() {
        let tsv = "# 주석\nthe\t100\n\ntheme\t40\n";
        let bytes = compile("en", tsv, None, false).unwrap();
    ...
    fn rejects_malformed_rows() {
        assert!(compile("en", "the 100", None, false).is_err());
        assert!(compile("en", "the\tabc", None, false).is_err());
        assert!(compile("en", "the\t0", None, false).is_err());
        assert!(compile("en", "", None, false).is_err());
        assert!(compile("en", "the\t100", Some("the quick 50"), false).is_err());
        assert!(compile("en", "the\t100", Some(""), false).is_err());
    }
}
