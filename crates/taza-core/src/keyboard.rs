...
pub use taza_pack::layout::{KeyAction, KeyboardLayout, LayoutKey, LayoutRow};
...
/// 레이아웃 + shift 상태에서 프레임을 만들고, 터치 좌표를 InputEvent로 판정한다.
/// 히트 테스트는 v1 기하 판정(포함 → 같은 행 최근접) — 확률(공간) 모델은 이 지점을
/// 교체하는 방식으로 들어온다.
pub struct Keyboard {
