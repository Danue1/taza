...
impl Engine {
    /// 언어팩을 갈아 끼운다. 팩이 스스로 밝힌 선언(표시 이름·입력 방식·조회 키 인코딩)이
    /// 내장 선언을 대신하고, 방식이 바뀌면 그 방식의 배열들이 함께 온다 — 언어를
    /// 늘리는 일이 팩 배포로 끝나되, 그 언어가 쓰는 방식은 이 빌드에 이미 있어야 한다.
    pub fn load_pack(&mut self, pack: Arc<dyn PackBytes>) -> Result<(), PackError> {
        ...
        if let Some(declared) = LanguageDescriptor::from_pack(&opened) {
            // 합성기 교체는 배열을 고른 뒤에 한다(`apply_layout_method`) — 방식을
        ...
        self.layouts = self.language.method.layouts();
    ...
    /// 배열이 자기 입력 방식을 밝혔으면 그 합성기로 갈아 끼운다 — 같은 언어 안에서도
    /// 조합 규칙이 다른 배열(천지인)이 있기 때문이다. 밝히지 않은 배열은 언어의 방식을
    /// 쓴다. 배열이 가리킬 수 있는 방식은 이 빌드에 링크된 것뿐이므로 빗나갈 일이 없다.
    ///
    /// 진행 중인 조합은 여기서 확정되지 않는다 — Effect를 낼 통로가 없기 때문이다.
    /// 배열·팩을 갈아 끼우기 전에 `CursorMoved`(또는 `FocusLost`)로 조합을 먼저 끝내는
    /// 것은 부르는 쪽의 몫이다.
    fn apply_layout_method(&mut self) {
        let declared = self
            .layouts
            .get(self.selected_layout)
            .and_then(|entry| entry.method);
        let method = declared.unwrap_or(self.language.method);
        if method == self.active_method {
            return;
        }
        self.composer = method.composer();
        self.active_method = method;
    }

    /// 배열이 바뀌어도 셸이 알려 준 표시 환경·설정·필드 성격은 이어진다.
    pub(super) fn rebuild_keyboard(&mut self) {
        self.apply_layout_method();
