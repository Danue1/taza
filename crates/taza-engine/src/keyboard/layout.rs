...
pub enum KeyAction {
    ...
    Multitap(Vec<char>),
    /// 자기 글자를 내지 않고 **직전 자모를 갈아 끼우는** 키 (나랏글의 획 추가·쌍자음).
    /// 코어는 marker를 평범한 글자 입력으로 흘리고 그 뜻은 합성기만 안다 — 그래서 이 키를
    /// 늘려도 판정·프레임·FFI에 갈래가 생기지 않는다. marker에 해당하는 글자가 유니코드에
    /// 없으므로 키에 적히는 말은 배열이 함께 댄다.
    Compose {
        marker: char,
        label: String,
    },
