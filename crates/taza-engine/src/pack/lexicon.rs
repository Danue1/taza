...
//! ```text
//! frequency u16 (0 = 단어 아님) | child_count u16 | (edge_byte u8, child_offset u32) × n
//! ```
...
impl<'bytes> Lexicon<'bytes> {
    ...
    fn node_frequency(&self, node: usize) -> Option<u32> {
        self.read_u16(node).map(u32::from)
    }

    fn node_children(&self, node: usize) -> Option<Vec<(u8, usize)>> {
        let count = self.read_u16(node + 2)? as usize;
        let mut children = Vec::with_capacity(count);
        for index in 0..count {
            let entry = node + 4 + index * 5;
