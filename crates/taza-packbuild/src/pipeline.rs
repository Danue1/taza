...
//! 주고, 그것을 어떻게 적을지는 부르는 쪽(`taza build`)이 정한다.
...
pub struct BuildOptions {
    /// 사람이 쓴 것이 사는 자리 — 레시피와 손으로 갖춘 원천. git이 추적한다.
    pub data_directory: PathBuf,
    /// 기계가 만든 것이 사는 자리 — 내려받은 원천, 추출 캐시, 중간 표, 팩, 카탈로그.
    /// 통째로 지워도 다시 만들어지는 것만 여기 있다.
...
/// 만들 언어들. 이름을 주지 않으면 디렉터리에 있는 것 전부를 사전순으로.
pub fn language_directories(
    options: &BuildOptions,
    names: &[String],
) -> Result<Vec<PathBuf>, String> {
    let languages = options.data_directory.join("languages");
    if !names.is_empty() {
        return names
            .iter()
            .map(|name| {
                let directory = languages.join(name);
                if directory.join("recipe.toml").exists() {
                    Ok(directory)
                } else {
                    Err(format!("레시피가 없음: {}", directory.display()))
                }
            })
            .collect();
    }
    let mut directories: Vec<PathBuf> = std::fs::read_dir(&languages)
        .map_err(|error| format!("{} 읽기 실패: {error}", languages.display()))?
        .filter_map(|entry| entry.ok().map(|entry| entry.path()))
        .filter(|path| path.join("recipe.toml").exists())
        .collect();
    directories.sort();
    if directories.is_empty() {
        return Err(format!("{}: 레시피가 없음", languages.display()));
    }
    Ok(directories)
}

/// 언어 하나를 팩과 배포 산출물까지 굽는다.
pub fn build(language_directory: &Path, options: &BuildOptions) -> Result<BuildOutcome, String> {
    let recipe = Recipe::load(language_directory)?;
    ...
    let signals: Vec<SourceSignal<'_>> = extracted
        ...
        .collect();
    let (words, lexicon_report) = normalize(&signals, &recipe.build.lexicon);
    let (bigrams, bigram_report) =
        normalize_bigrams(&signals, &words, &recipe.build.language_model);

    let build_directory = options.output_directory.join("build");
    write_tables(
        &build_directory,
        &recipe,
        &words,
    ...
    let packs_directory = options.output_directory.join("packs");
    ...
    let entry = CatalogEntry {
        ...
        language: recipe.pack.language.clone(),
        pack_version: recipe.pack.pack_version,
...
type Extraction<'recipe> = (&'recipe Source, taza_corpus::Signal, bool);

fn extract<'recipe>(
    recipe: &'recipe Recipe,
    options: &BuildOptions,
) -> Result<Vec<Extraction<'recipe>>, String> {
    let cache = options.output_directory.join("cache");
    ...
    let prepared: Vec<Result<Prepared, String>> = std::thread::scope(|scope| {
        let handles: Vec<_> = recipe
            ...
            .iter()
            .map(|source| {
                let (cache, language) = (&cache, &recipe.pack.language);
                scope.spawn(move || {
                    source::prepare(
                        &source.file,
                        &source.name,
