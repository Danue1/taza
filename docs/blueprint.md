...
- 후보 바 자리도 코어가 정한다: 예측을 내지 않는 필드에서는 `candidate_bar_height`가 0이라
  ...
  자리를 남긴다.
- **다중 문자 키**(`KeyAction::Text`)와 **빈 자리**(`KeyAction::Blank`)를 팩 포맷에 더했다
  (와이어 kind 8·9, DSL은 여러 글자 토큰과 `blank`). `.com`은 어절을 끊으므로 조합을 먼저
  확정하고 넣는다.
...
- 배치는 세벌식 최종(3-91)을 그대로 옮겼다(libhangul `hangul-keyboard-3f` 기준). 데스크톱에서
  ...
  비웠다.
- **잔여**: 한국어 팩은 번들에 없으므로(내장은 영어뿐) 세벌식은 팩을 받은 뒤에야 기기에서
  고를 수 있다 — 팩 배포가 선행 조건이다. 실기기 타건 검증은 미완.
...
아니라 **배열도** 밝힐 수 있게 했다(`NamedLayoutSet::skeleton`, 레이아웃 섹션 marker 1,
DSL `=== <이름> : <골격>`). 청사진이 예약해 둔 "한 언어에 복수 Composer 등록"이 이 통로다.
...
- **멀티탭은 배열 데이터 + 시간이다**(`KeyAction::Multitap`, 와이어 kind 10, DSL `ㄱ|ㅋ|ㄲ`):
...
- **잔여**: 세벌식과 마찬가지로 한국어 팩을 받아야 고를 수 있고, 실기기 타건 검증은 미완.
  나랏글(획 추가·쌍자음)은 손대지 않았다.
...
- **한 언어가 배열을 여러 벌 싣는다**(팩 포맷: 레이아웃 섹션 맨 앞의 0이 배열 목록 표시,
  옛 팩은 그대로 읽힌다). DSL은 `=== <이름>`으로 배열을 열고, **레이어를 덜 적은 배열은
  첫 배열의 나머지 레이어를 물려받는다** — 배열끼리 다른 것은 대개 문자면뿐이라
  심볼면 세 벌이 배열마다 복사되지 않는다. 영어 팩이 QWERTY·QWERTZ·AZERTY·Colemak을
  싣고, 고른 배열은 `LanguagePreferences`가 이름으로 기억한다(팩 갱신으로 사라진 이름은
  조용히 기본 배열로 돌아간다).
- DSL에서 시프트 표기를 적지 않은 글자는 **대문자로 올라간다**. 대소문자가 없는
