...
- **구현됨 (taza-toolchain, `taza-packs`)**: 레시피 하나로 조달부터 배포 산출물까지 돈다 —
  `data/recipes/<언어>.toml`(원천 URL·sha256·라이선스·추출 규칙·크기 예산) → 다운로드·해시
  검증·캐시(`data/cache`) → 원천 형식별 추출기 → 병합·필터·점수 정규화 → 팩 조립 →
  zstd 아카이브 + `catalog.json` + `NOTICE.md` 자동 생성. 언어 추가는 레시피 한 장이고,
  새 원천 형식만 추출기 하나(`extract.rs`)를 늘린다.
