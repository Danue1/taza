...
| 한국어 | mecab-ko-dic(Apache-2.0) 형태소 사전 + 위키·공개 말뭉치(모두의 말뭉치는 이용 조건 확인 필수) | **어절 단위 사전은 교착어 특성상 폭발** — 형태소 단위 LM + 조사·어미 결합 모델로 설계. 오타 모델은 자모 레벨. **자모 레벨 v1 구현됨**: 사전을 자모 분해 + 두벌식 ASCII로 인코딩 저장(`--hangul-jamo`)해 trie 바이트 OSA = 자모 편집거리. HangulComposer가 어절 자모 추적 → 완성·거리1 교정·개인화 부스트·선택 치환. 두벌식 인접성 오타 합성 + 한국어 평가 게이트 포함. 형태소 LM·자동교정은 미착수 |
