...
필드가 지정하는 초기 언어(순정은 이메일·URL에서 라틴 배열로 열고, 비밀번호에서는
언어 전환 자체를 막는다) / 리턴키 라벨의 로컬라이즈(지금은 "검색"이 한국어 고정) /
한국어 복수 배열(천지인·나랏글·세벌식) — 배열 선택 골격은 갖췄고 남은 것은 합성기다
(천지인은 멀티탭 타이머, 세벌식은 초성·종성을 구분하는 자모 오토마톤).
