...
- **필드가 초기 언어를 지정하지 않는다.** 순정은 이메일·URL을 라틴 배열로 열고 비밀번호
  필드에서는 언어 전환 자체를 막는다. 우리는 언어 목록을 셸이 소유하므로(`ShellRequest`)
  코어가 "이 필드는 라틴으로 열라"고 요청하는 통로가 먼저 필요하다.
- **리턴키 문구가 한국어 고정**이다("검색"). 라벨은 코어가 정하는데 코어에 UI 언어 개념이
  아직 없다.
- **`inputmode="none"`을 갈래로 두지 않았다.** 키보드를 띄울지 말지는 시스템이 정하므로
  서드파티 키보드가 할 일이 없다 — 웹 셸이 생기면 그때 다시 본다.
- **`enterkeyhint`의 나머지 문구**(go·send·next 등)는 매핑하지 않았다. iOS는 `returnKeyType`
  으로 오고 Android는 `IME_ACTION_*`으로 오므로 갈래를 늘리기 전에 두 쪽 이름을 맞춰야 한다.
