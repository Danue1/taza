#!/bin/bash
# 언어팩을 GitHub Release로 올린다. `taza build`를 먼저 돌려 산출물을 만들어 두어야 한다.
#
# 올리는 것은 `out/packs`(생성물)이고, 릴리스 본문으로 쓰는 고지는 `data/notices`(저장소에
# 남는 것)에서 온다 — 그래서 이 스크립트는 저장소 뿌리에 있다.
#
...
packs="${repository_root}/out/packs"
notice="${repository_root}/data/notices/NOTICE.md"

if [[ ! -f "${packs}/catalog.json" ]]; then
    echo "카탈로그가 없다: ${packs}/catalog.json — 먼저 taza build를 돌린다" >&2
    exit 1
fi
